...
it('constructs with an empty item list', () => {
	const matrix = new ComparisonMatrixMap([]);
	expect(matrix).toBeDefined();
});

it('initializes 1x1 with EQ token', () => {
	const matrix = new ComparisonMatrixMap(['only']);
...
it('initializes 3x3 with EQ tokens', () => {
	const matrix = new ComparisonMatrixMap(['a', 'b', 'c']);
	// A = A, B = B, C = C
...
it('returns expected results for example scenario', () => {
	const matrix = new ComparisonMatrixMap(['good', 'best', 'better', 'worst', 'worse']);
